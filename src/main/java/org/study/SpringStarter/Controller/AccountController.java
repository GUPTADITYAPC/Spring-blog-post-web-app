...
import org.study.SpringStarter.services.EmailService;
import org.study.SpringStarter.util.AppUtil;
import org.study.SpringStarter.util.email.EmailDetails;
...
public class AccountController {
    ...
    public String updatePhoto(@RequestParam("file") MultipartFile file, RedirectAttributes attributes,
        ...
        if (file.isEmpty()) {
            ...
            try {
                ...
                attributes.addFlashAttribute("success", "Photo uploaded successfully");
    ...
    public String reset_password(@RequestParam("email") String _email, RedirectAttributes attributes, Model model) {
        ...
        if (optional_account.isPresent()) {
            ...
            accountService.save(account);
            String reset_message = "To reset your password, please click the link below:\n" + site_domain
                    + "change-password?token="
                    + reset_token
                    + "\n\nIf you did not request a password reset, please ignore this email.";
            String subject = "Password Reset Request";
            EmailDetails emailDetails = new EmailDetails(account.getEmail(), reset_message, subject);
            if (emailService.sendSimpleEmail(emailDetails) == false) {
                attributes.addFlashAttribute("error",
                        "Failed to send reset email. Please try again later or contact admin.");
                return "redirect:/forgot-password";
            }

            attributes.addFlashAttribute("message", "Password reset link has been sent to your email");
            return "redirect:/login";
        } else {
            attributes.addFlashAttribute("error", "No account found with that email");
            return "redirect:/forgot-password";
        }
    ...
    public String change_password(Model model, @RequestParam("token") String token, RedirectAttributes attributes) {
        if (token.equals("")) {
            attributes.addFlashAttribute("error", "Token is required");
            return "redirect:/forgot-password";
        }
    ...
    public String post_change_password(
            @ModelAttribute Account account, RedirectAttributes attributes) {
